/**
 * File header: Minimal POSIX ustar tar writer + deterministic gzip used to package a small set of
 * files (export bundle assets, an emitted KiCad library) into a single downloadable `.tar.gz`.
 *
 * EE Library does not pull in a third-party tar/zip dependency for this; the archives are small
 * (a handful of CAD/datasheet files plus a manifest/readme), every entry is a regular file, and the
 * paths are short engineering-style paths that fit comfortably in tar's 100-byte name field.
 * Anything that does not fit is rejected up front so the caller gets a clear failure rather than a
 * silently-truncated archive.
 *
 * Lives in shared so both the API (inline KiCad library emission) and the worker (export-bundle
 ...
 * not supported here because these archives use short engineering-style paths.
 */
 ...
 * current time into the gzip header, which makes byte-identical archives regenerate to differing
