...
// Unique per run so retries and dirty stacks never collide with an earlier onboarded project.
const runStamp = Date.now().toString(36);
const droppedFolderName = `e2e wizard ${runStamp}`;
const renamedFolderName = `E2E-WIZARD-${runStamp.toUpperCase()}`;
...
test.beforeAll(async () => {
  ...
  // Drop one "old project" folder into the mirror exactly as an engineer would: as-is,
  // messy name and all, with a parts list whose MPN column is recognizable.
...
test.afterAll(async () => {
  ...
  // Onboarding renames the folder, so clean up both possible names.
  await rm(path.join(projectFilesDir, droppedFolderName), { force: true, recursive: true });
  await rm(path.join(projectFilesDir, renamedFolderName), { force: true, recursive: true });
});

// Sign in once per test; the journey steps share the authenticated browser context.
...
test("engineer drops a folder, scans, adds it to the library, and sees the honest outcome", async ({ page }) => {
  // --- Scan the project files root from /projects -------------------------------------------
  await page.goto("/projects");
  await page.getByRole("button", { name: "Scan for project folders" }).click();

  // The dropped folder appears with its parts list and the disclosed rename.
  const folderRow = page.locator("tr", { hasText: droppedFolderName });
  ...
  await expect(page.locator("tr", { hasText: renamedFolderName })).toHaveCount(0);
});
