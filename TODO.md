...
1. **RBAC expansion** — _Phase 1–2 shipped 2026-05-23 (foundation):_ role set widened to `viewer / contributor / approver / admin` (legacy `user` kept as full-power baseline) via `packages/shared` `AppRole` + migration `044`, plus a `Permission` policy and a single `requirePermission` chokepoint in `apps/api/src/auth.ts` (`requireAdmin` now routes through `governance.admin`). Behavior unchanged — only admin-vs-non-admin is enforced; no engineering route migrated yet; no account restricted. _**Paused 2026-05-23**_ (small, fully-trusted team today — see memory `rbac-team-usage`): remaining is **Phase 3** per-route permission migration (decide legacy-`user` posture + `/sign-up` exposure first), per-project scope, the `reviewer`/`exporter` roles, and the role-assignment admin UI. Resume when onboarding someone who should not have full power. Honesty: roles decide who *may* act; the shipped audit log records what they *did*.
...
5. **Real ECAD/MCAD emission** — _First slice shipped 2026-05-23 (packaging-only):_ deterministic **KiCad library emission** from a project's verified, file-backed assets — merged `.kicad_sym`, `.pretty` footprints, STEP 3D, generated lib tables + provenance README, packaged as `.kicad-lib.tar.gz` (`packages/shared/src/kicad-library.ts`, `apps/worker/src/kicad-library-emission.ts`, CLI `npm run emit:kicad-library <projectId>`). Packages only — never generates geometry; only `verified_for_export` assets are included. _Remaining:_ a web download action + async emission pipeline (mirror export-bundle assembly), optional footprint 3D-path rewriting, then a **SolidWorks add-in** and KiCad round-trip. Generated/packaged content stays `generated`, never `official`.
