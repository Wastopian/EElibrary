...
- **Portable engineering-memory archive (you own your data)** — `npm run export:engineering-memory -- --out backup.tar.gz` streams every database table plus referenced storage files into one deterministic `.tar.gz` with a manifest (format/schema version, row counts, per-file hashes). `npm run import:engineering-memory -- --in backup.tar.gz [--dry-run]` restores it: it refuses a schema-version mismatch, restores tables in FK order in one transaction, and uses `ON CONFLICT DO NOTHING` so it **never overwrites** existing rows (storage files are written only when absent). A two-command backup + disaster-recovery round trip for a self-hosted team.
...
- **Real ECAD/MCAD emission** — deterministic **KiCad library emission** (packaging verified assets into a drop-in `.kicad-lib.tar.gz`) is shipped: a one-click "Build KiCad library" download on the project export panel, an admin-gated API route, and the worker CLI (`npm run emit:kicad-library <projectId>`). An async pipeline for very large projects and a SolidWorks add-in are still planned.
...
npm run import:engineering-memory -- --in backup.tar.gz --dry-run
```
