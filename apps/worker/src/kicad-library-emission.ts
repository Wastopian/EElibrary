/**
 * File header: Worker-side KiCad library emission.
 *
 * Reads a project's verified, file-backed KiCad assets, hands them to the deterministic emitter in
 * `@ee-library/shared`, copies 3D model bytes, and packages the result into a single drop-in
 * `.kicad-lib.tar.gz` written to storage. This is **packaging**, not generation: only assets that are
 * `export_status = 'verified_for_export'` with stored bytes are included, so the trust boundary
 * (imported ≠ approved ≠ export-ready, generated ≠ official) is preserved end to end.
 *
 * The DB query, the pure row grouping, and the storage-bound packaging are split so the grouping and
 * packaging can be unit-tested without a database.
 */

import { createHash } from "node:crypto";
import { basename } from "node:path";
import { emitKicadLibrary, type KicadEmissionPart, type KicadEmissionResult } from "@ee-library/shared/kicad-library";
...
/**
 * Emits a KiCad library for one project end to end: query verified assets, group, package, and write
 * the archive to storage. Returns an `empty` summary (no archive written) when the project has no
 * verified file-backed CAD assets, so callers can surface an honest "nothing to export yet" state.
 */
