...
| System health | `SYSTEM_ARCHITECTURE.md` | Shipped | `apps/api/src/system-health.ts`, `packages/shared/src/system-health-types.ts`, `apps/web/src/components/WorkerStatusBanner.tsx`, `apps/web/src/app/system/page.tsx`, `AppNavigation.tsx` | Reports API, database, storage, worker heartbeat, and async queue state. `/system` is a sidebar workspace with service cards, queue counts, raw endpoint access, and recovery links so health is discoverable even when the banner is quiet. |
