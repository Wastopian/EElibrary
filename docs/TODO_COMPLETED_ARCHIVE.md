# Completed backlog archive (frozen snapshot)

**Source:** migrated **2026-05-05** from the repository root [`TODO.md`](../TODO.md) when the active backlog was split out.

**Use this file for:** FUNC/MEM completion narratives, regression checklists, execution-plan history, and “what we shipped” prose.

**Do not use as:** the current task list — open work lives only in root [`TODO.md`](../TODO.md). Shipped-vs-planned truth lives in [`IMPLEMENTATION_STATUS.md`](./IMPLEMENTATION_STATUS.md).

---

# EE Library — Engineering Memory TODO (historical export)

**Snapshot updated:** 2026-05-05 (through FUNC18 complete)  
**Working branch:** `main` (includes merge of `cursor/2026-05-03-7o4u-787f9` engineering-memory track)  
**Historical purpose:** Track shipped part-readiness and engineering-memory foundation work.
...
- Connector set catalog view ships at `/connector-sets`: browsable by `connector_class`, with best-mate / alternate-mate pairs from `mate_relations` and confirmed project usage counts per connector and mate (FUNC15).
- Approval batch workflow ships on the project detail page: explainable approval-gap queue with bulk approve / flag-for-review actions that record project context and only update part-level approval rows (FUNC16).
- Inline **stored PDF preview** on part detail engineering asset cards when preview is ready and the file is local (`downloaded`/`validated`); reference-only PDFs show honest copy instead of a fake preview (FUNC17).
- **`/compare`** workspace for up to four catalog parts (`?parts=id1,id2,…`), plus sidebar nav and part-detail “Compare workspace” link (FUNC18).
...
- ~~Approval batch workflow from project BOM context.~~ Shipped via FUNC16.
- ~~Connector set catalog view browsable by family and mate pairs.~~ Shipped via FUNC15.

The original FUNC1–FUNC16 engineering-memory backlog is shipped; FUNC17–FUNC18 add catalog UX depth (preview + compare). Broader **tools/calculators** and **deep compare** (connector/CAD-first matrices) remain planned — see `docs/IMPLEMENTATION_STATUS.md` and `docs/ENGINEERING_MEMORY_ROADMAP.md`.

---

## Recent completed backlog items (2026-05-05)

- **Catalog trust badging — completed (2026-05-05)**
  - Catalog dense table adds a **Trust gates** column; list cards include the same four abbreviated badges under readiness summary.
  - Uses shared lineage logic with tooltips so imported/reviewed/approved/verified-for-export stay visually distinct from aggregate trust score alone.

- **Asset preview (continued) — second increment (2026-05-06)**
  - Added image formats to shared `FileFormat` (`png`/`jpg`/`jpeg`/`webp`) and inline stored-image preview rendering.
  - Added worker-side preview-status guard so `ready` is persisted only for embeddable local artifacts.

- **Deeper compare (continued) — second increment (2026-05-06)**
  - `/compare` now renders per-asset trust-stage diff rows by engineering asset class.
  - Generated/approved/verified differences are now directly scannable side-by-side.

- **Usability follow-on — compare flow + project usage readability (2026-05-06)**
  - Added in-page compare selection tray (add/remove ids) and catalog compare actions to reduce URL-edit friction.
  - Updated project confirmed-usage table to MPN/manufacturer-first identity display, preserving internal IDs as secondary context.

- **Export path hardening (continued) — completed**
  - Export bundle creation now writes a deterministic archive payload to storage when available.
  - Bundle rows now surface storage write failures as explicit manifest warnings.
  - `/storage/:key` now requires admin auth before file serving.

- **Validation and trust surfacing (continued) — completed**
  - Admin review/promotion queues now show compact trust context from the four-stage lineage.
  - Project usage rows now surface compact trust context from persisted snapshots.
  - Stage boundaries remain explicit (imported/reviewed/approved/verified-for-export are not collapsed).
...
- **Doc sync (post-FUNC16)** — Updated **2026-05-04**: `docs/IMPLEMENTATION_STATUS.md`, `README.md` (“What Ships Today”, planned vs shipped, boundaries), and `docs/ROADMAP.md` (forward-looking priority) now match the shipped engineering-memory surface (matching, usage, where-used, BOM health, evidence vault, circuit blocks, `/connector-sets`, approval batch, etc.). Use `IMPLEMENTATION_STATUS.md` as the live contract; avoid re-stating the full matrix in `TODO.md`.
- Code ships the full project/BOM memory track listed in “Current Reality” above; the P0-MEM narrative sections below are **historical** completion notes, not open work.
- **Next engineering wave** (per `AGENTS.md`, also reflected in `docs/ROADMAP.md`): **FUNC17–FUNC18** (stored PDF preview + basic compare) are complete. Next: **export path hardening**, **validation/trust surfacing**, **3D/STEP or image preview** where honest, **tools/calculators**, and **deeper compare** (connector + CAD matrices). Track in `IMPLEMENTATION_STATUS.md` when work begins.
...
### P2-FUNC15 — Connector Set Catalog View — DONE 2026-05-04
...
**Completion notes**:
- Shared types: `ConnectorSetMatePairKind`, `ConnectorSetMatePair`, `ConnectorSetEntry`, `ConnectorSetClassGroup`, `ConnectorSetListResponse` with explicit boundary copy.
- Store: `readConnectorSetCatalogFromDatabase(filters?)` reads `parts` joined to `manufacturers`, `part_readiness_summaries`, `part_approvals`, and an aggregated `project_part_usages` count, then loads mate rows from `mate_relations` (best_mate + alternate_mate) for each matched connector and groups the results by `connector_class`. Optional filters: `connectorClass`, `query` (MPN/manufacturer substring). Bounded by `CONNECTOR_SET_MAX_CONNECTORS = 200`. No new schema is created.
- API route: `GET /connector-sets[?connectorClass=…&q=…]`. Telemetry classifier `api-connector-set-list`.
- Web client: `fetchConnectorSetCatalog(filters?)`.
- UI: new `/connector-sets` workspace mounted under shell navigation. Renders a hero snapshot (connectors / mate pairs / classes / project usages), a class+query filter form, per-group lists of connector cards, and a per-connector mate table with relationship type, confidence score, mate part state, and mate project usage count. Trust-boundary section makes it explicit that listing connectors does not approve reuse, validate evidence, or unlock export.
- Tests: pg-mem `mate_relations` table added to fixtures; new `seedConnectorCatalogRowsViaPool` helper seeds JST connector + best/alt mates + crimp tool. Test asserts groups, mate-pair counts, confidence values, the `connectorClass` and `query` filter behavior, and the boundary copy.

### P2-FUNC16 — Approval Batch Workflow From Project BOM Context — DONE 2026-05-04
...
**Completion notes**:
- Shared types: `ApprovalBatchAction` (`approve` | `flag_for_review`), `ApprovalBatchCandidate`, `ApprovalBatchCandidatesResponse`, `ApprovalBatchRequest`, `ApprovalBatchOutcome`, `ApprovalBatchOutcomeStatus`, `ApprovalBatchResponse` with explicit boundary copy.
- Store: `readApprovalBatchCandidatesFromDatabase(projectId)` builds the candidate set from matched-status BOM lines and confirmed `project_part_usages` for the project, aggregates BOM line counts and designators in JS, joins to catalog identity + approval state, and excludes parts already `approved`. `applyApprovalBatchInDatabase(projectId, input, decidedBy)` validates input (`PART_IDS_REQUIRED` / `INVALID_ACTION` / `TOO_MANY_PART_IDS` capped at 200), upserts only `part_approvals` rows with project-context evidence (`project:<id>`, `project_key:<key>`, `triggered_by:approval_batch`, `decided_by:<actor>`), and never touches `part_readiness_summaries`, `assets`, or `part_issues`. Per-part outcome statuses: `applied`, `skipped_already_approved`, `skipped_no_change`, `not_found`.
- API routes: `GET /projects/:projectId/approval-candidates` and admin-gated `POST /projects/:projectId/approval-batch`. Telemetry classifiers `api-approval-batch-candidates` and `api-approval-batch-apply`. 400 on invalid input, 404 on missing project.
- Web client: `fetchApprovalBatchCandidates(projectId)` and `applyApprovalBatch(projectId, input)`.
- UI: new `ApprovalBatchPanel` mounted on the project detail page (Section 10 — “Review approval gaps”), under BOM health. Loads candidates, supports per-row select / select-all / clear, action selector (`approve` / `flag_for_review`), optional notes, submit, and renders a per-part outcome summary plus the trust boundary copy.
- Tests: three new pg-mem cases — candidates query excludes already-approved parts and surfaces a non-approved matched resistor, `applyApprovalBatchInDatabase` records an applied / skipped_already_approved / not_found outcome and writes project-context summary while readiness and assets remain untouched, and validation rejects empty `partIds`, an unsupported action, and a missing project.

### P1-FUNC17 — Stored PDF asset preview on part detail — DONE 2026-05-05

**Priority**: P1 (aligned with `AGENTS.md` asset registry / preview)

**Why it matters**: Engineers should see datasheet evidence without guessing whether “preview ready” is decorative.

**Expected outcome**:
- When an asset row is `previewStatus: ready`, `fileFormat: pdf`, and availability is locally stored (`downloaded` or `validated`), the UI embeds the existing API download route in a bounded iframe.
- Reference-only or missing-file PDFs never fake an iframe; explicit copy points to Download / View source.

**Completion notes**:
- New `apps/web/src/components/AssetInlinePreview.tsx` (`canEmbedAssetPreview`, `AssetInlinePreview`) wired into `EngineeringAssetSummary` on `apps/web/src/app/parts/[partId]/page.tsx`.
- Uses `buildAssetDownloadUrl(partId, asset.id)` (redirect-capable GET already exposed by the API).
- Styles: `globals.css` (`.asset-inline-preview`, frame height bounds).

### P1-FUNC18 — Part compare workspace — DONE 2026-05-05

**Priority**: P1 (aligned with `AGENTS.md` compare priority)

**Why it matters**: Engineers pick between alternates using the same normalized metrics and readiness signals they trust elsewhere—not screenshots.

**Expected outcome**:
- `/compare?parts=id1,id2,id3,id4` loads up to four parts and renders summary + union of normalized metrics with honest blanks.
- Navigation includes Compare; part detail links into compare with the current part id.

**Completion notes**:
- New `apps/web/src/app/compare/page.tsx` (dynamic SSR), `apps/web/src/lib/part-compare.ts`, `buildCompareUrl` in `apps/web/src/lib/api-client.ts`.
- `AppNavigation` adds Compare; part detail adds `detail-nav-links` with **Compare workspace**.
- Tests: `apps/web/src/lib/part-compare.test.ts` (metric union + missing cell).
...
- Connector set catalog view: `/connector-sets` lists connectors grouped by `connector_class` with mate pairs and project usage counts, no new schema required (FUNC15).
- Approval batch workflow from project BOM context: explainable approval-gap queue with bulk approve / flag-for-review actions and project-context evidence (FUNC16).
...
**P0-FUNC1** through **P0-FUNC5**, **P1-FUNC6**–**P1-FUNC8**, and **P1-FUNC9**–**P2-FUNC16** are complete.

Goal: every shipped FUNC item now ladders up to a real daily-use engineering workflow — revision comparison, export download UI, circuit block instantiation, fleet health, lifecycle regression, substitution sign-off, connector-set browsing, and project-scoped bulk approval all run from persisted internal state. The next planning wave is open; see `docs/ENGINEERING_MEMORY_ROADMAP.md`.
...
- Do not present any future-tense FUNC items as shipped until implemented; the FUNC1–FUNC16 wave is now complete and any new claims belong to the next backlog wave.
