...
async function main() {
  ...
  // Check vague-query rejection on POST /parts/import
  const vagueImportResponse = await safeFetch("/parts/import", {
    body: JSON.stringify({ mpn: "regulator" }),
    headers: { "Content-Type": "application/json" },
    method: "POST"
  });
  if (vagueImportResponse.status === 400 && vagueImportResponse.body?.status === "rejected" && vagueImportResponse.body?.reason === "vague_query") {
    check("POST /parts/import (vague)", "pass", "vague query was rejected with reason=vague_query");
  } else {
    check("POST /parts/import (vague)", "fail", `expected 400 vague_query, got HTTP ${vagueImportResponse.status} ${JSON.stringify(vagueImportResponse.body)}`);
  }
